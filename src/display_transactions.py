...
def show_transactions(transaction_dict):
    ...
    date = change_date(transaction_dict['date'])
    description = transaction_dict['description']

    if 'from' in transaction_dict:
        sender_number = transaction_dict['form']
        hidden_sender_number = sender_number.replace(sender_number.split()[-1], hide_sender_number(sender_number))
    else:
        hidden_sender_number = ''

    recipient_number = transaction_dict['to']
    hidden_recipient_number = recipient_number.replace(recipient_number.split()[-1], hide_recipient_number(recipient_number))
